...
from pydantic import BaseModel, Field, validator, StrictStr, StrictBool, StrictInt, Extra
import jsonschema
...
async def document_resource_put(
    ...
    for entry in data:
        ...
        if isValid is False:
            raise HTTPException(
                500, "schema mismatched for entry: " + str(entry) + "\n" + err
            )
...
@router.get(
    "/configs/{config}/d/{document}/e/{entry}/v/",
    tags=[Tags.congifs]
)
